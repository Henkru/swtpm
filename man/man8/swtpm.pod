...
=item B<--tpmstate dir=E<lt>dirE<gt>|backend-uri=E<lt>uriE<gt>[,mode=E<lt>0...E<gt>][,lock][,backup][,fsync]>
...
If I<backup> is specified then the TPM storage backend will make a backup of the
permanent state file every time the state is rewritten. The backup file can be
used to replace a corrupted permanent state file.

When swtpm is started with this option and the permanent state file is missing
but the backup file exists, then swtpm will rename the backup file to the
permanent state file and try to start with it. In this case the file-renaming is
permanent even if swtpm cannot read the file contents (corrupted, wrong
decryption key). Otherwise, it will try to start with the permanent state file
first and if this fails, it will try to start with the backup file. If both
attempts fail, the original state of the files will be restored.

The I<backup> option is only supported by the directory-backend and is
rejected by other storage backends.

If I<fsync> is specified then the directory storage backend will call fsync
on the file and the directory of the file whenever state is written to disk.
This option ensures that all data have been successfully written to physical
storage before the TPM processes the next command. Using this option lowers
the probability of TPM state file corruption in case of a power loss. Please
also see the man page for fsync.

Note that this option may have an impact on the performance of the TPM for
any command that requires that the TPM's state be written.

The I<fsync> option can be combined with the I<backup> option or can be used
as an alternative to the I<backup> option.
...
TPM/TPM2_SetLocality commands. This parameter is useful if the Linux
...
=item B<tpmstate-dir-backend-opt-backup> (since v0.11)

The option parameter I<backup> for the I<--tpmstate> option is supported
for the directory storage backend.

=item B<tpmstate-dir-backend-opt-fsync> (since v0.11)

The option parameter I<fsync> for the I<--tpmstate> option is supported
for the directory storage backend.

=item B<cmdarg-lua>, B<lua-api-1> (optional build feature)

The socket TPM 2 frontend supports Lua interception API version 1 when built
with C<--with-lua>. See C<--lua-script> below.

=item B<cmdarg-pcap> (since v0.11)

The option I<--pcap> is supported to write TPM command and response exchanges
into a pcapng-formatted file.

=item B<systemd-notify> (since v0.11)

systemd's readiness notification protocol is supported, see below.

=back

=item B<--print-states> (since v0.7)

This option allows the user to print out the TPM 1.2 or TPM 2 state blobs
...
This option allows the user to control the locking of the NVRAM storage
...
This option allows the user to set a profile for a TPM 2. A built-in profile can be
...
The I<fd=> option parameter allows the user to read it from a file descriptor.
...
=item B<--lua-script PATH> or B<--lua-script-fd N>

Load one Lua 5.4 text script before accepting traffic. Requires a build configured
with C<--with-lua> and C<swtpm socket --tpm2>. Only raw TPM 2 framing is supported,
including QEMU's passed data FD; the optional TCG prefix is rejected. The source
must be a regular file, limited to 1 MiB. FD arguments must be at least 3.

The script returns a table containing integer C<api_version = 1> and at least one
of C<on_request(ctx, bytes)> and C<on_response(ctx, bytes)>. Missing callbacks,
C<nil> returns, and C<{action="pass"}> pass traffic unchanged.
C<{action="replace", bytes=STRING}> replaces the request or response.
The request callback may return C<{action="respond", bytes=STRING}> to bypass
execution and invoke the response callback once on a synthetic reply. Strings
are binary-safe. Replacement messages must have a supported TPM 2 tag, an exact
declared length, and fit the negotiated buffer limit. Authentication is not
recomputed.

C<ctx> contains C<id>, C<generation>, C<tpm_version>, C<locality>,
C<command_code>, C<original_request>, C<transmit_limit> (64), and a mutable
per-exchange C<state> table.
The response callback additionally receives C<effective_request>,
C<effective_command_code>, C<backend_executed>, and C<response_source>
(C<tpm>, C<swtpm>, or C<lua>). Synthetic exchanges have no effective request.
Metadata is copied afresh for each callback; replacing C<ctx.state> does not
replace the retained state table. Globals persist until process exit, including
across TPM initialization generations. Automatic Startup/Shutdown bypass Lua.

Lua is limited to 16 MiB and one million instructions per initialization or
callback. Restricted base, string, table, and math operations are available;
filesystem/process, coroutine, and debug libraries are absent. C<require> and the
package library's preload and Lua-file searchers are available. C<package.path>
uses Lua's normal environment and compiled defaults and may be changed by the
main script; relative entries are resolved against swtpm's working directory.
Native modules remain disabled: C<package.loadlib> is absent, C<package.cpath> is
empty, and the C searchers are removed. Loading outside C<require>, protected-call
and metatable functions are removed. Lua 5.4 C<string.pack>, C<string.unpack>,
and native bitwise operators are available for binary parsing.
Scripts remain trusted inputs:
instruction limits are not wall-clock isolation for C library functions.
Script errors, invalid returns, budget violations, and capture/delivery failures
stop swtpm with a nonzero exit status, retaining partial artifacts.

The global C<tpm> table provides command execution and in-memory helpers. All byte
arguments and results are binary Lua strings, including embedded zero bytes;
numbers are not converted to strings. Each crypto function returns one binary
string on success. Bad arguments or cryptographic failures raise a Lua error
and are recorded as fatal interception failures.

=over 4

=item C<response, rc = tpm.transmit(command)>

Execute one complete raw TPM 2 command synchronously on the same TPM instance
and return its complete binary response and the integer TPM response code from
the response header. Only calls made during C<on_request> or C<on_response>,
including through Lua helper functions, are allowed. Calls during script
initialization are errors. Exactly one argument, a binary string, is required.
Its tag must be C<0x8001> or C<0x8002>, its declared size must match the string
length, and it must fit the negotiated buffer limit. Responses must also have
valid framing and fit that limit.

Additional commands execute immediately, in call order, and bypass both Lua
callbacks and swtpm-local command handling such as SetLocality. They inherit the
enclosing exchange's locality. In C<on_request> they finish before the effective
external request executes; in C<on_response> they finish before the final
external response is delivered. This also works in handlers that produce or
receive synthetic replies, provided the TPM is running and storage is available.
No automatic Startup or retry is performed. A nonzero TPM response code is a
normal return for the script to handle; invalid arguments, unavailable TPM or
storage, libtpms processing failures, invalid responses, and log failures are
fatal interception errors.

There are at most 64 additional commands per handler invocation, independently
for the request and response handlers. C<ctx.transmit_limit> reports this limit;
changing the context does not change enforcement. Returned strings count toward
the Lua memory limit. The command count and Lua instruction limits do not bound
wall-clock time spent executing TPM commands.

Responses from additional commands go only to Lua. To deliver one to the client,
explicitly return it in a C<respond> or C<replace> action. Additional commands
do not change the enclosing exchange's context metadata, including
C<backend_executed> and C<response_source>. They do change real TPM state,
including handles, sessions, and persistent state. Changes are not rolled back
if a subsequent command or handler fails or the handler returns a synthetic
reply. Authentication is not recomputed for either additional or external
commands. See F<samples/lua/transmit.lua> for an example.

=item C<digest = tpm.sha256(data)>

Return the 32-byte SHA-256 digest. C<data> may be empty and is limited to 1 MiB
(1048576 bytes).

=item C<mac = tpm.hmac_sha256(key, data)>

Return the 32-byte HMAC-SHA-256 value. C<key> is limited to 1 KiB (1024 bytes)
and C<data> to 1 MiB. Either may be empty.

=item C<plaintext = tpm.rsa_oaep_sha256_decrypt(private_der, ciphertext, label)>

Decrypt an exactly 256-byte RSA-2048 ciphertext using OAEP with SHA-256 for both
OAEP and MGF1. C<private_der> must contain one complete, unencrypted PKCS#8
RSA-2048 private key, with no trailing DER, and must be nonempty and at most
4 KiB (4096 bytes). The modulus must be positive, odd, and 2048 bits. The
decrypted session salt must be exactly 32 bytes; other lengths are errors.

=item C<ciphertext = tpm.rsa_oaep_sha256_encrypt(modulus, exponent, plaintext, label)>

Encrypt an exactly 32-byte session salt using RSA-2048 OAEP with SHA-256 for
both OAEP and MGF1. C<modulus> is exactly 256 bytes encoding an unsigned,
big-endian, odd, 2048-bit integer. C<exponent> must be a Lua integer, odd and
between 3 and 4294967295 inclusive. The script must translate a TPM-encoded
exponent of zero to 65537 before calling this function. The result is exactly
256 bytes; encryption uses OpenSSL randomness.

For both RSA functions, C<label> is required and may contain at most 64 bytes,
including any zero bytes. It is used exactly as supplied, without adding or
removing a terminator. For example, the Lua string C<"SECRET\0"> supplies seven
bytes. An empty label is valid.

=item C<plaintext = tpm.aes_128_cfb_decrypt(key, iv, ciphertext)>

Decrypt with AES-128-CFB128 without padding. C<key> and C<iv> must each be
exactly 16 bytes. C<ciphertext> is limited to 1 MiB, may be empty, and need not
be block-aligned. The result has exactly the same length as the ciphertext.

=item C<tpm.log(message)>

The supported output API; global C<print> is unavailable. C<message> must be a
binary string. Its exact bytes are recorded as hex in a JSONL C<log> event when
C<--lua-log-fd> is supplied, and a readable form is mirrored to stderr (which
may stop at an embedded zero byte). The cumulative budget is 4096 bytes per
initialization or callback, with each empty message charged one byte. Exceeding
the budget or a write failure is fatal.

=back

Crypto argument limits bound work and allocations inside OpenSSL, outside
Lua's instruction counter. Returned strings count toward the 16-MiB Lua memory
limit. Temporary C plaintext and key material are cleaned up, but script-supplied
private DER and returned plaintext remain in Lua memory until released or
process exit; Lua does not guarantee that released string storage is wiped.
Required Lua files are read when first requested and cached in C<package.loaded>;
they are not included in the primary script snapshot or its SHA-256 digest. No
general file I/O, native dynamic loading, or additional Lua libraries are enabled.

=item B<--lua-log-fd N>

Write incremental interception JSONL to a writable regular file. Records include
source SHA-256, the exact source snapshot as hex, API/Lua versions, build source
provenance, limits, original/effective requests, original/final responses,
exchange IDs, initialization generations, monotonic timestamps, internal-command
origin, processing and write results, diagnostics, and completion counters.
Additional C<tpm.transmit> commands have C<origin="lua"> and C<id=0>, with
C<parent_id> identifying the external exchange, C<phase> identifying
C<on_request> or C<on_response>, and a one-based C<sequence> shared across both
handlers and reset for each external exchange. Request, processing, and response
records preserve call order. The processing C<result> is the libtpms status;
the response C<result> is the TPM response code when a response is present.
C<backend_executed> on these records describes the additional command.
The manifest includes C<transmit_limit>; the run summary includes
C<transmit_calls> (validated calls admitted to the budget, including failed
attempts) and C<transmit_responses> (valid backend responses, including TPM errors).
Unreached stages have null payloads. The launcher can decode the snapshot to
C<attack.lua> and merge provenance into its own C<manifest.json>.

=item B<--lua-pcap-fd N>

Write a second pcapng capture to a writable regular file. This capture contains
only external requests dispatched to C<TPMLIB_Process()> and its original
responses. Synthetic and swtpm-local replies produce no backend exchange.
Internal automatic commands remain only in native capture and the event log.
Additional C<tpm.transmit> commands and their responses are recorded only in
the event log; they are excluded from both pcap captures. An additional response
explicitly returned to the client appears as the final response in native capture.
An empty backend capture is valid for an all-synthetic run.

Artifact FDs are optional but recommended for recorded experiments, must refer
to distinct files, and require a script. The Lua bridge duplicates inherited FDs
and closes the originals after initialization and the copies on cleanup.
Retain stderr and process exit status as well as artifacts:
a successful socket write does not prove guest consumption, and missing exchange
or run completion records indicate an incomplete run. Capture/log write and close
failures fail Lua runs. See F<samples/lua/> for examples.

=item B<--pcap file=<path>|fd=<file descriptor>[,truncate][,mode=E<lt>0...E<gt>][,checksums]>

This option allows writing TPM command and response exchanges to a
pcapng-formatted file. If truncate is passed, then an existing file is
truncated otherwise the new data will be appended. The mode option allows
to choose the file mode bits. The default file mode bits are 0640.
When the checksum parameter is passed, then IP and TCP headers will carry
correct checksum. By default the checksums in the headers are set to zero.

Swtpm will write a simulated TCP message exchange to the pcap file. The TPM's
port is 2321 and the source port is a random port in the range [50000 .. 55000].
The destination port of 2321 will enable TPM packet decoding in Wireshark for
example.

Swtpm will also write packets that are not explicitly sent to the TPM by the
user, but those that are sent internally, such as for example a I<TPM2_Startup>
when I<--flags startup-clear> is used.

Without Lua interception, swtpm silently ignores errors related to writing the
pcap file, such as when no space is available. Lua runs propagate these failures
to their exit status.
...
B<swtpm> supports systemd's readiness notification protocol. If the
I<NOTIFY_SOCKET> environment variable is set, B<swtpm> will send a
readiness notification (C<READY=1>) to it once it is ready to accept
commands. This allows the use of C<Type=notify> in systemd service unit
files. Both filesystem and abstract AF_UNIX sockets are supported.
